...
class LlamaDecoderLayer(nn.Module, GradientCheckpointingLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
...
class LlamaEmbedding(nn.Module):
    ...
    def cuda(self, device=None):
        super(LlamaEmbedding, self).cuda(device=device)
        self.wpe.cuda(device=device)
        return self

    def encrypt(self, mode=True, src=0):
        super(LlamaEmbedding, self).encrypt(mode=mode, src=src)
...
@auto_docstring
class LlamaModel(nn.Module, LlamaPreTrainedModel):
...
@auto_docstring
class LlamaForCausalLM(nn.Module, LlamaPreTrainedModel, GenerationMixin):
