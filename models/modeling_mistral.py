...
class MistralDecoderLayer(nn.Module, GradientCheckpointingLayer):
    def __init__(self, config: MistralConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class MistralEmbedding(nn.Module):
    ...
    def cuda(self, device=None):
        super(MistralEmbedding, self).cuda(device=device)
        self.wpe.cuda(device=device)
        return self

    def encrypt(self, mode=True, src=0):
        super(MistralEmbedding, self).encrypt(mode=mode, src=src)
...
@auto_docstring
class MistralModel(nn.Module, MistralPreTrainedModel):
    def __init__(self, config: MistralConfig):
        ...
        self.embed_tokens = MistralEmbedding(config.vocab_size, config.hidden_size, self.padding_idx)
...
@auto_docstring
class MistralForCausalLM(nn.Module, MistralPreTrainedModel, GenerationMixin):
